...
app.layout = html.Div([
    ...
    style={"backgroundColor":"#f0f0f0"} # set background to grey
)
...
def update_scatter_plot(selected_big_scape_class, selected_distance_metric, selected_average_distance, selected_color_scale, selected_hybrids, search_value):
    ...
    try: # Because they can select Hybrid class without having Hybrids separated selected
        if selected_key in dictDFMDS_ALL['dictDFMDS_' + selectedHybridVal + 'HybridCategory'][selected_big_scape_class+'_orig']:
            ...
            # Search-based outlines:
            if search_value:
                df['IsSearchMatch'] = df.index.str.contains(search_value, case=False)
                fig.update_traces(marker=dict(line=dict(color='black',width=[3 if is_search_match and search_value else 0.5 for is_search_match in df['IsSearchMatch']])))
            else:
                # Set outlines to white for non-matching points
