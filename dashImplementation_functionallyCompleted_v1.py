...
# This assumes you've already run the BiG-SliCE.
...
print('The path to the data.db file in the results folder: ' + pathToDataDB) 
pathToReportsDB = "C:/Users/imraa/Downloads/DataS10-bigsliceOutputs-antiSMASH-v511/DataS10-bigsliceOutputs-antiSMASH-v511/full_run_result/reports/";
print('The path to the reports folder: ' + pathToReportsDB)
run_id = 6; # This is the BiG-SLiCE data associated with the Threshold = 900. If you queried your BGCs against the 29,955 GCFs, runID 6 is what you want.
print('The run_id used for BiG-SLiCE (6 is associated with Threshold = 900): ' + str(run_id))
thresholdValue = 900; # This is the BiG-SLiCE Threshold that I used, which queried BGCs agaisnt teh 29,955 GCFs.
print('The threshold used for BiG-SLiCE processing: ' + str(thresholdValue))
...
print('Strains to remove from the BiG-SLiCE visualizations: ' + strainsToRemoveB)
    # Note: Look for this bit of code. You can modify this as you see fit.
    # remove a1363, b482, and b486 (if you want to remove c500, just add it to the string seen below "c500|a1363|b482|b486")
    # storageDF38 = storageDF1.copy(deep=True);
    # storageDF38 = storageDF38[storageDF38['Genome'].str.contains(strainsToRemoveB)==False]; # if the Genome column has these strings, remove those rows

# change genome name to be WMMC250 rather than c250: FIND THIS TEXT AND COMMENT IT IF YOU DON'T WANT WMM to be added before every strain name.
    # storageDF38['Genome'] = 'WMM' + storageDF38['Genome'];
    # storageDF38['Genome'] = [i.upper() for i in storageDF38['Genome']]

# Requires that the BiG-SLiCE runs be named:
    # "m6dfas.demux.bc1021_genome.contigs/ + 'thresh900_29955ranks_antismash511'"
    # So, an example one might be: "m6d.demux.bc1021_GENOMENAME.contigs/thresh900_29955ranks_antismash511"

# The relevant folders in BiG-SLiCE that you want to analyze.
...
print('Output of scripts: ' + scriptOutputs)

# Path to csv file containing BGCs x GCFs (BiG-SLiCE): Used to minimize resource usage after running Section 5 once.
try:
    queriedBGC_GCFpath = 'C:\\Users\\imraa\\Documents\\UWM\\BugniLab\\Genome Assembly\\scriptOutputs\\antismash5-strainsQueried-rank.csv';
except:
    print('No previously analyzed data regarding BiG-SLiCE queries run through this program already exists...')
# Path to excel file containing the MIBIG BGCs and my BGCs, and their associated bgc product categories according to BiG-SCAPE classes (+ Hybrid (Not PKS/NRPS)):
# bigSCAPEclassesAllpath = r'C:\Users\imraa\Documents\UWM\BugniLab\Yeast-Chemical-Genomics\annoCytoscape-v5-fullReplacedNames.xlsx';
try:
    bigSCAPEclassesAllpath = 'C:/Users/imraa/Documents/UWM/BugniLab/Genome Assembly/scriptOutputs/20231211_annoCytoscape-v511-fullReplacedNames_noMIBiG.xlsx'
except:
    print('No information regarding antiSMASH analysis from already running this code exists...')
...
try:
    superDF3 = pd.read_csv('C:\\Users\\imraa\\Documents\\UWM\\BugniLab\\Genome Assembly\\scriptOutputs\\20231211-antismash5-strainsQueried-rank.csv');
    ...
    # superDF2T.to_csv('20231211-antismash5-strainsQueried-rank.csv');
...
# classList = [];
# [classList.append(bgSCAPEclasses.loc[bgSCAPEclasses['name 1'] == i]['bgc product'].tolist()[0]) for i in testT1];
# # Make sure I have max 10 classes (because that's how many I should have under my scheme)
# nClasses = len(unique(classList));
...
#%% Flask App
chdir(r'C:\Users\imraa\Documents\UWM\BugniLab\Flask-Implementation\Dissertation')
...
app.layout = html.Div([
    ...
    html.H3("Welcome to the Dash Tool to Assist with Prioritizing BGCs Based on Potential Novelty", style={'textAlign':'center'}),
    html.Div(
        id="intro",
        children="Manipulate the various dropdown bars to customize which data is being presented, the metadata used to color the plots, and the associated color scales.", style={'textAlign':'center'}),    
    # Dropdowns for user input
    html.P("BGC Class: Choose from pre-defined BGC classes", style={'textAlign': 'center'}),
    dcc.Dropdown(
        id='bigScapeClass-dropdown',
        # options=[{'label': key, 'value': key} for key in dictDFMDS.keys()],
        options=[{'label':key,'value':key} for key in substrings],
        # value=list(dictDFMDS.keys())[0],  # Default value
    ...
    dcc.Dropdown(
        ...
        options=[
            ...
            # for metric in dictDFMDS['NRPS_orig'].keys()
            for metric in distMets
        ],
        # value=list(dictDFMDS['NRPS_orig'].keys())[0],  # Default value
    ...
    dcc.Dropdown(
        ...
        options=[
            {'label': metric, 'value': metric} for metric in [1,2,3,4,5]],
    ...
    dash.html.Center(dcc.Graph(id='scatter-plot'))  # Plotly graph
])
...
def update_scatter_plot(selected_big_scape_class, selected_distance_metric, selected_average_distance, selected_color_scale):
    # selected_key = f"{selected_big_scape_class}_orig_{selected_distance_metric}"
    try:
        selected_key = f"{selected_big_scape_class}_orig_{selected_distance_metric}"
    except:
        return px.scatter() # Return empty plot if key is not found.
    
    if selected_key in dictDFMDS[selected_big_scape_class+'_orig']:
        # Title manipulation
        bgcTitleName = selected_key.split('_')[0]
        metricTitleName = selected_key.split('_')[-1].capitalize()
        df = dictDFMDS[selected_big_scape_class+'_orig'][selected_key]
        # Choose which average distance to use.
        selected_aveDist = 'Average of ' + str(selected_average_distance) + ' Distances'
        # Generate plot
        fig = px.scatter(df, x='x', y='y', color_continuous_scale=selected_color_scale,color=selected_aveDist,size=selected_aveDist,hover_data = ['Metadata','Distance'], 
                         hover_name = ['WMM'+i.capitalize() for i in df.index.tolist()])#, # Cleaned up BGC names. 
                         # title=f'{bgcTitleName} with {metricTitleName} Distance Metric <br><sup>Size & Color Described by Average Distance of BGC to Closest {selected_average_distance} GCFs</sup>') # title=f'Visualization for {selected_key}')
        # Adjust plot size
        fig.update_layout(height=1000,width=1200)#,margin=dict(auto=True))
        fig.update_layout(title_text = f'{bgcTitleName} BGCs Analyzed with {metricTitleName} Distance Metric <br><sup>Size & Color Described by Average Distance of BGC to Closest {selected_average_distance} GCFs</sup>', title_x=0.5)
        fig.update_layout(xaxis_title= 'Dimension 1', yaxis_title='Dimension 2')
        return fig
    else:
        return px.scatter()  # Return an empty plot if the key is not found
